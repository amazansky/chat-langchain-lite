...
def test_setup_guide_returns_guide_when_topic_names_the_product():
    assert "LangGraph Platform" in get_setup_guide.invoke({"topic": "langgraph deployment"})
...
def test_security_advice_answers_in_scope_query():
    assert "RECOMMENDED patterns" in get_security_advice.invoke(
        {"query": "langgraph checkpointer security"}
    )
