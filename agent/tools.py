...
# These knowledge bases only cover the LangChain ecosystem, so a lookup whose
# argument names nothing in it has to report out-of-scope rather than hand back
# the nearest LangChain answer as if it applied.
...
def get_setup_guide(topic: str) -> str:
    """Get a setup/how-to guide (installation, environment, deployment, evaluation) for the LangChain / LangGraph / LangSmith / Deep Agents ecosystem ONLY — the topic must name the product, e.g. "langgraph deployment"; never call this for other frameworks, cloud providers, or general infrastructure topics."""
